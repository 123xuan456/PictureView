...
public class ImageDetailFragment extends Fragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (!TextUtils.isEmpty(mImageUrl)) {
            ...
            options = (new RequestOptions()).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(mImageLoading).error(mImageLoading);
            Glide.with(getActivity()).asBitmap().load(mImageUrl).apply(options).thumbnail( 0.1f ).into(new SimpleTarget<Bitmap>() {
                @Override
